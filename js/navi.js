...
const RU_MODEL_LOW = [
	// 2026-07-25 〜
	// [b, a, 右端, 次モデルの左端]
	[100.0000000000, 3.16084360e+00],  // __0k ..  5k  0.80576 1649
	[471.5818090200, 3.08652724e+00],  // __5k .. 10k  0.93238 1756
	[2332.077706820, 2.97314307e+00],  // _10k .. 15k  0.86712  927
	// 49704 46929, 48030
];

{
    // RU_MODEL_LOW の右端と次モデルの左端を計算して追加
    for (let i = 0; i < RU_MODEL_LOW.length; i++) {
        const [b, a] = RU_MODEL_LOW[i];
        const x = (i + 1) * 5000;
        const [b2, a2] = (i + 1 < RU_MODEL_LOW.length) ? RU_MODEL_LOW[i + 1] : RU_MODEL_BA[0];
        RU_MODEL_LOW[i].push(Math.floor(b + a * x));	// 右端
        RU_MODEL_LOW[i].push(Math.floor(b2 + a2 * x));  // 次モデルの左端
    }
}
